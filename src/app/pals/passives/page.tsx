...
export default function PassiveSearch() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row gap-4 bg-slate-800/50 p-4 rounded-lg border border-slate-700">
        <input
          type="text"
          placeholder="Search passives..."
          className="flex-1 px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500"
          value={search}
        ...
        {filteredPassives.map((passive) => (
          <div
            ...
            className={`
              ...
              ${passive.tier === 3 ? 'bg-yellow-900/20 border-yellow-500/50 shadow-yellow-900/10' :
                passive.tier === 2 ? 'bg-slate-800/80 border-amber-200/30' :
                'bg-slate-800/50 border-slate-700'}
            `}
          >
            <div className="flex justify-between items-start mb-2">
              <h3 className={`font-bold ${
                passive.tier === 3 ? 'text-yellow-400' :
                passive.tier === 2 ? 'text-amber-200' :
                'text-slate-200'
              }`}>
                {passive.name}
              </h3>
              <span className={`text-[10px] px-1.5 py-0.5 rounded border ${
                passive.tier === 3 ? 'bg-yellow-900/40 border-yellow-700 text-yellow-200' :
                passive.tier === 2 ? 'bg-amber-900/30 border-amber-800 text-amber-200' :
            ...
            <div className="space-y-1 pt-3 border-t border-white/5">
              {Object.entries(passive.stats).map(([stat, val]) => (
                <div key={stat} className="flex justify-between text-xs">
                  <span className="capitalize text-slate-500">{stat.replace(/([A-Z])/g, ' $1').trim()}</span>
